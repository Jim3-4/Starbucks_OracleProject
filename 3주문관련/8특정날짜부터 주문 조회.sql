--특정날짜부터 주문 조회
CREATE OR REPLACE PROCEDURE up_vieworderDate
(
  pmember_id orders.member_id%TYPE
  ,porder_date orders.order_date%TYPE
...
BEGIN
   ...
   vsql := vsql || 'WHERE member_id=:member_id AND order_date BETWEEN :order_date AND SYSDATE ';
   vsql := vsql || 'ORDER BY o.order_no DESC, od.oo_no DESC ';


   OPEN  vcursor FOR vsql USING pmember_id, porder_date;
...
--ID, 날짜
--1년전 까지 출력
EXEC  up_vieworderDate('jaeyoung',SYSDATE - INTERVAL '1' YEAR);    
--