...
패키지를 사용한 이유는 유사한 업무에 사용되는 여러 개의 프로시저나 함수를 하나의 패키지로 묶어서  관리해야 자신이 필요한 항목을 찾는게 더 수월해지기 때문입니다. 패키지를 사용하면 자유 사용되는 프로그램과 로직을 모듈화 할 수  있기 때문에 유지보수 작업이 편해지고 프로그램의 처리흐름을 노출되지 않기에 보안기능이 향상될 수 있습니다.
 <br> <br>
pay_sudan5 ⇒파라미터를 통해 전달받은 주문번호를 통해 스타벅스카드(포인트와 같은 개념)에서 차감하는 프로시저입니다. 

if문을 통해서 결제금액이 잔액보다 작거나 같다면 잔액을 갱신하고 

차감문구를 띄웁니다. 

(pay테이블의 card_no 칼럼에는 스타벅스카드만 데이터값이 들어가 있습니다. 스타벅스카드만 데이터 값이 들어간 이유는 스타벅스에서 스타벅스카드의 의미는 포인트와 같기 때문에  스타벅스측에서 관리하기 때문입니다. )
<br>

pay_sudan6-⇒ pay_method에서 스타벅스카드를 제외한 신용카드 ,쿠폰, ssgpay에서  파라미터로 입력받은 주문번호의 결제방법과 금액을 가져옵니다. if문을 통해서 pay_method에 따라 다르게 처리합니다.
### 관리자 
